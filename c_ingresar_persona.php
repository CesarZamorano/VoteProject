...
if (isset($_POST['guardar']))
	{
		...
		('$rut', '$nombre', '$direccion', '$telefono', '$clave', '$comuna', '$colegio', '3', '$sexo', '0', '2', $nro_documento)";
		//echo $query;
...
<tr>
   ...
   <td>Comuna</td>
    ...
    <td><select name="colegio" style="height:25px; border-radius: 10px;width: 240px">
	        <option value="0">---Seleccione una opción---
	        <?php
			$sqlSelect="select rbd, nombre from colegio where rbd=$rbd";
			$resultSelect =mysql_query($sqlSelect,$enlace);
			while ($fila=mysql_fetch_row($resultSelect))
			{
				echo"<option value='".$fila['0']."'>".$fila['0']." -- ".$fila['1'];
			}
			?>
            </select></td>
    </tr>
    <small>*Si desea ser candidato, debe inscribirse directamente en el Servicio Local, presentando la documentación solicitada.</small>
<!--Revisar el -->
	 <!--<tr>
    <td>¿Es candidato?</td>
