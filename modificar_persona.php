...
if (isset($_POST['guardar']))
{
	...
	$n_sexo=$_POST['sexo'];
    $n_colegio=$_POST['colegio'];
    $n_candidato=$_POST['candidato'];
...
if (isset($_POST['modificar']))
{
	$nfilas=0;
	$rut=$_POST['rut'];
	$query="select * from persona where rut=$rut and id_tipo_usu='2'" ;
	...
	{
		$sqlsel="select * from persona where rut=$rut and id_tipo_usu='2'";
		$res=mysql_query($sqlsel,$enlace);
...
<tr>
   ...
   <td>Comuna</td>
    ...
    <tr>
    <td>Colegio</td>
    <td><select name="colegio" style="height:25px; border-radius: 10px;width: 240px">
            <option value="0">---Seleccione una opción---
            <?php
            $sqlSelect="select rbd, nombre from colegio";
            $resultSelect =mysql_query($sqlSelect,$enlace);
            while ($fila=mysql_fetch_row($resultSelect))
            {
                if($fila['0']==$id_colegio)
                echo"<option value='".$fila['0']."' selected>".$fila['0']." -- ".$fila['1'];
                else
                echo"<option value='".$fila['0']."'>".$fila['1'];
            }?>
            </select></td>
     ...
     <tr>
    <td>¿Es candidato?</td>
    <td><select name="candidato" style="height:25px; border-radius: 10px;width: 240px">
            <option value="0">---Seleccione una opción---
            <?php
            $sqlSelect="select * from tipo_ciudadano";
            $resultSelect =mysql_query($sqlSelect,$enlace);
            while ($fila=mysql_fetch_row($resultSelect))
            {
                if($fila['0']==$id_ciudadano)
